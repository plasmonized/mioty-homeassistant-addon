...
**Version: 1.0.6.14** - NEW: SCACI-Direktverbindung (TLS) zum Service Center als zusätzliche Option neben MQTT — vollständiger SCACI v1.0.0 Client (app/scaci_client.py, msgpack-Framing "MIOTYA01"), Einstellungs-UI mit AC-EUI-Generator, Zertifikats-ZIP-Upload (certs/ac_&lt;eui&gt;/), Statusanzeige, Uplinks fließen in die normale Sensor-Pipeline, Base-Station-Status-Poll alle 60 s. Zuvor: Versionsanzeige unter Einstellungen („ℹ️ Über"), veraltete Versionsnummern aus Headern entfernt, Sensor-Dropdown auch bei Decoder-Zuweisung. Zuvor: ifm EIO240 als zweiter IO-Link Adaptertyp (gerätetyp-abhängige Payload-Auswertung), Sensor-Dropdown bei Adapter-Registrierung, Duty-Cycle-Bargraph in der Base-Station-Übersicht, Ingress-404-Fix für Dashboard-Link. Zuvor: Re-Anlern-Buttons (einzeln + bulk) auf der Sensoren-Seite via neuem Endpoint `/api/sensor/reregister` (nutzt gespeicherte Network Keys aus sensor_configs.json). FIX: Sensor-Registrierung nutzt jetzt QoS 1 mit Broker-Bestätigung (PUBACK); stille MQTT-Verluste werden als Fehler gemeldet. Integrated IoddProcessParser for automated IODD interpretation. Added IO-Link Adapter management with IODD assignment per adapter. New Decoder page with 3 sections: Decoder, IO-Link Adapter, IODD Management.

...
**Version: 1.0.5.7.11** - NEW: Re-Anlern-Buttons (einzeln + bulk) auf der Sensoren-Seite via neuem Endpoint `/api/sensor/reregister` (nutzt gespeicherte Network Keys aus sensor_configs.json). Zuvor: FIX: Sensor-Registrierung nutzt jetzt QoS 1 mit Broker-Bestätigung (PUBACK); stille MQTT-Verluste werden als Fehler gemeldet. Integrated IoddProcessParser for automated IODD interpretation. Added IO-Link Adapter management with IODD assignment per adapter. New Decoder page with 3 sections: Decoder, IO-Link Adapter, IODD Management.

...
function decode(dataBytes, metadata) {
    ...
    if (ver_min < 3) {
        ...
        if (ver_min == 1) {
            if (result.status === 1 && dataBytes.length >= 140) {
                ...
                result.e_t1_a_i_Wh = fetchUint64(true);
                result.e_t2_a_i_Wh = fetchUint64(true);
                result.e_t1_a_e_Wh = fetchUint64(true);
                result.e_t2_a_e_Wh = fetchUint64(true);
                result.e_t1_r_i_Wh = fetchUint64(true);
                result.e_t2_r_i_Wh = fetchUint64(true);
                result.e_t1_r_e_Wh = fetchUint64(true);
                result.e_t2_r_e_Wh = fetchUint64(true);
                result.i_l1_mA = fetchSint32(true);
                result.i_l2_mA = fetchSint32(true);
                result.i_l3_mA = fetchSint32(true);
                result.i_l4_mA = fetchSint32(true);
                result.i_l123_mA = fetchSint32(true);
                result.p_l1_a_W = fetchSint32(true);
                result.p_l2_a_W = fetchSint32(true);
                result.p_l3_a_W = fetchSint32(true);
                result.p_l123_a_W = fetchSint32(true);
                result.p_l123_a_avg_W = fetchSint32(true);
                result.u_l1_V = fetchSint32(true) / 10;
                result.u_l2_V = fetchSint32(true) / 10;
                result.u_l3_V = fetchSint32(true) / 10;
                result.f_Hz = fetchSint16(true) / 10;
                index += 16;
                result.pf_l1 = fetchSint8(true) / 10;
                result.pf_l2 = fetchSint8(true) / 10;
                result.pf_l3 = fetchSint8(true) / 10;
            } else if (result.status === 2) {
                result.e_ta_a_i_Wh = fetchUint64(false);
                result.e_ta_a_e_Wh = fetchUint64(false);
            }
        } else if (ver_min == 2) {
            if (result.status === 1) {
                result.p_l1_a_W = fetchSint32(false);
                result.p_l2_a_W = fetchSint32(false);
                result.p_l3_a_W = fetchSint32(false);
                result.p_l123_a_W = fetchSint32(false);
                result.i_l1_mA = fetchSint32(false);
                result.i_l2_mA = fetchSint32(false);
                result.i_l3_mA = fetchSint32(false);
                result.i_l123_mA = fetchSint32(false);
                result.u_l1_V = fetchSint32(false) / 10;
                result.u_l2_V = fetchSint32(false) / 10;
                result.u_l3_V = fetchSint32(false) / 10;
                result.e_ta_a_i_Wh = fetchUint64(false);
                result.e_ta_a_e_Wh = fetchUint64(false);
                result.e_ta_r_i_Wh = fetchUint64(false);
                result.e_ta_r_e_Wh = fetchUint64(false);
                result.pf_l1 = fetchSint8(false) / 100;
                result.pf_l2 = fetchSint8(false) / 100;
                result.pf_l3 = fetchSint8(false) / 100;
                result.f_Hz = fetchSint16(false) / 10;
    ...
    if (result.payload_profile === 0) {
        ...
        result.u_l2_V = fetchSint32(false) / 10;
        result.u_l3_V = fetchSint32(false) / 10;
        result.u_l12_V = fetchSint32(false) / 10;
        result.u_l23_V = fetchSint32(false) / 10;
        result.u_l31_V = fetchSint32(false) / 10;
        result.e_ta_a_i_Wh = fetchUint64(false);
        result.e_ta_a_e_Wh = fetchUint64(false);
        result.e_ta_r_i_Wh = fetchUint64(false);
        result.e_ta_r_e_Wh = fetchUint64(false);
